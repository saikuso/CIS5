...
void sort(vector <int> & fill_random); // Bubble Sort
...
int main ()
{
	...
	cout << "The vector is being generated with random numbers.\n" << endl;
	
	// Displays the output of random numbers (pre-sorted)
	displayList(fill_random);
	
	// Executes Bubble Sort on vector 'fill_random'
	sort(fill_random);
	
	// Progress Marker (Vector Sorted)
...
void displayList (vector <int> fill_random)
{
	for (int i=0; i< fill_random.size(); i++)
	{
		cout << fill_random[i] << ", ";
		if ((i + 1) % 10 == 0)
		{
...
void sort (vector <int> & fill_random)
{
	bool swapped = false;
	int holder;
...
// Swap Array Elements Function (Not needed)
